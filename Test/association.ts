...
describe('Testing Associations and Methods ', () => {
  ...
  it('returns associated data for existing belongsTo association', async () => { 
    ...
    const db = await ConnectDb(); 
    let q1 = await db.queryObject(`SELECT * FROM users WHERE firstname='user_one';`)
    let q2 = await db.queryObject(`SELECT * FROM users WHERE firstname='user_two';`)
    let user1 = Object.assign(new User(), q1.rows[0])
    let user2 = Object.assign(new User(), q2.rows[0])
    await db.queryObject(`UPDATE users SET team_id=1 WHERE id='${user1.id}'; UPDATE users SET team_id=1 WHERE id='${user2.id}';`)
    q1 = await db.queryObject(`SELECT * FROM users WHERE firstname='user_one';`)
    q2 = await db.queryObject(`SELECT * FROM users WHERE firstname='user_two';`)
  ...
  it('forms hasMany association based on existing belongsTo relationsip, then returns associated data', async () => {
    ...
    const db = await ConnectDb(); 
    const t1 = await db.queryObject(`SELECT * FROM teams WHERE teamname='team A';`)
  ...
  it('returns associated data based on existing Many-To-Many association', async () => {
    const db = await ConnectDb(); 
    const m1 = await db.queryObject(`SELECT * FROM members WHERE name='member_one';`)
    const c1 = await db.queryObject(`SELECT * FROM clubs WHERE clubname='Tennis Club';`)
