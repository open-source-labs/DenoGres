...
import { Planet, Person } from '../sample_model.ts';
...
describe('model methods', () => {
  ...
  describe('transaction', () => {
    it('it should commit single query to the database upon successful transaction for single table', async() => {
      ...
      const planet = await db.queryObject(`SELECT name FROM planets WHERE name = 'turtle'`)
    ...
    it('it should commit multiple queries to the database upon successful transaction for a single table',async()=>{
      ...
      await Planet.endTransaction();
      const planetAlex = await db.queryObject(`SELECT name FROM planets WHERE name = 'Alex'`);
      const planetJacob = await db.queryObject(`SELECT name FROM planets WHERE name = 'Jacob'`)
      const planetMia= await db.queryObject(`SELECT name FROM planets WHERE name = 'Mia'`)
      const planetRachel = await db.queryObject(`SELECT name FROM planets WHERE name = 'Rachel'`)
    ...
    it('it should commit a single query per table to the database upon successful transaction for two table', async() => {
      ...
      await Planet.endTransaction();
      const planet = await db.queryObject(`SELECT planets.name FROM planets INNER JOIN people ON planets.name = people.name`);
    ...
    it('it should commit multiple queries to the database upon successful transaction for multiple tables',async()=>{
      ...
      const Hyperion = await db.queryObject(`SELECT name FROM planets WHERE name = 'Hyperion'`);
      const Endymion = await db.queryObject(`SELECT name FROM planets WHERE name = 'Endymion'`)
      const Shrike = await db.queryObject(`SELECT name FROM people WHERE name = 'Shrike'`)
      const Consul = await db.queryObject(`SELECT name FROM people WHERE name = 'The Consul'`)
