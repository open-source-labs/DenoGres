...
export const modelParser = (): ModelInfo[] => {
  ...
  const cleanedText = modelText.replaceAll(
    /export interface \w+ *\{[\n* *.*: \w+,*]+\}/g,
    "",
  )
    .replaceAll(
  ...
  tableArray.forEach((el) => {
    ...
    props.forEach((prop) => {
        ...
        columnsArray.forEach((column) => {
          ...
          while (colInfo.length && entered) {
            ...
            if (colInfo.slice(0, 7) === "length:") {
              ...
              columnsObj.length = Number(
                colInfo.split(",")[0].replaceAll(/\}/g, ""),
              );
            ...
            if (colInfo.slice(0, 9) === "enumName:") {
              ...
              columnsObj.enumName = colInfo.split(",")[0].replaceAll(
                /^\'|^\"|\'$|\"$/g,
                "",
              );
              colInfo = colInfo.replace(String(columnsObj.length), "").replace(
            ...
            if (colInfo.slice(0, 7) === "checks:") {
              ...
              const value = colInfo.split(/(?=\")]/, 1)[0]
                ...
                .split(/\' *, *\'|\" *, *\"/)
                .map((element) => {
                  element = element.replaceAll(/^\"\(|^\'\(/g, "").replaceAll(
                    /\)\"|\)\'/g,
                    "",
                  )
                    .replaceAll(/^\'|\'$|^\"|\"$/g, "");
            ...
            if (colInfo.slice(0, 11) === "defaultVal:") {
              ...
              if (columnsObj.type === "boolean") {
                ...
                [
                  ...
                  "float4",
                  "float8",
                ].includes(columnsObj.type)
              ) {
                transformValue = Number(value);
              } else if (
                value.replace(/,$/, "").slice(-3) === "()'" ||
                value.replace(/,$/, "").slice(-3) === '()"'
              ) {
                transformValue = value.replace(/,$/, "").replaceAll(
                  /^\'|^\"|\'$|\"$/g,
                  "",
                );
              } else {
            ...
            if (colInfo.slice(0, 12) === "association:") {
              ...
              const tableVal = colInfo.split(/\, *mappedCol\: */)[0].replaceAll(
                /^\'|^\"|\'$|\"$/g,
                "",
              );
              ...
              const mappedColVal = colInfo.split(/\,* *\} */)[0].replaceAll(
                /^\'|^\"|\'$|\"$/g,
                "",
              );
              colInfo = colInfo.replace(mappedColVal, "").replace(
                /\'\'|\"\"/,
                "",
              ).replace(/ *} */, "");

              columnsObj.association = {
                table: tableVal,
                mappedCol: mappedColVal,
              };
            }
          }
      ...
      if (prop.slice(0, 6) === "checks") {
        const checksArray = prop.replace(/checks *= */, "").replaceAll(
          /^\[|\]\}$|\]$/g,
          "",
        ).split(/\' *, *\'|\" *, *\"/);
        ...
        const pkArray = prop.replace(/ *primaryKey *= *\[/, "").replaceAll(
          /\]$|\]\}$/g,
          "",
        ).split(",");
        tableObj.primaryKey = pkArray.map((element) =>
          element.replaceAll(/^\"|^\'/g, "").replaceAll(/\"|\'/g, "")
        );
      } else if (prop.slice(0, 6) === "unique") {
        const uArray = prop.replace(/ *unique *= *\[/, "").replace(
          /\]$|\]\}$/,
          "",
        ).split(/\] *, *\[/);
        tableObj.unique = uArray.map((element) =>
          element.replaceAll(/^\[|\]$/g, "").replaceAll(/\"|\'/g, "").split(",")
        );
        ...
        fkArray.forEach((elFK) => {
          ...
          elFKArray.forEach((attr) => {
            if (attr.includes("columns:")) {
              ...
              table = attr.replaceAll(/ *table: */g, "").replaceAll(
                /^\'|^\"|\'$|\"$/g,
                "",
              );
            }
          ...
          if (
            Array.isArray(tableObj.foreignKey) && columns && mappedColumns &&
            table
          ) {
