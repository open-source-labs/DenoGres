...
export const enumSync = async () => {
  ...
  dbEnum.forEach((el) => {
    if (typeof el === 'object' && el !== null && enumRowGuard(el)) {
      dbEnumObj[el.enum_name] = {
        enum_schema: el.enum_schema,
        enum_value: el.enum_value.split(/, */),
      };
  ...
  Object.keys(modelEnum).forEach((key) => {
    ...
    if (!Object.keys(dbEnumObj).includes(key)) {
      ...
      if (
        JSON.stringify(modelEnum[key]) !==
          JSON.stringify(dbEnumObj[key].enum_value)
      ) {
          ...
          rev.forEach((val, idx) => {
            ...
            if (!tempDBEnumVal.includes(String(val))) {
              if (idx === 0) {
                ...
                enumCreateAlter +=
                  `ALTER TYPE ${key} ADD VALUE '${val}' BEFORE '${
                    rev[idx - 1]
                  }';`;
              }
            }
