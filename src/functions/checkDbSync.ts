...
export function checkDbSync(): void {
  uniqueLog('db-sync');
  const info =
    `This model was created when the command --db-sync was invoked on ${todaySync}.
  ...
  ensureDir('./Migrations')
    ...
    .then(() => {
      ...
      Deno.writeTextFileSync(
        `./Migrations/syncedModel_${dateFolderSync}/synced_build.ts`,
        modelAfter,
      );
      Deno.writeTextFileSync(
        `./Migrations/syncedModel_${dateFolderSync}/synced_build.txt`,
        modelAfter,
      );
      Deno.writeTextFile(
        `./Migrations/syncedModel_${dateFolderSync}/model_changes.txt`,
        info,
      );
    });
}

export { dateFolderSync, todaySync };
